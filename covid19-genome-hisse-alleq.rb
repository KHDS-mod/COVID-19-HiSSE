...
q_obscomm ~ dnExp(qpr)
moves.append( mvScale(q_obscomm, lambda=0.50, tune=true, weight=3.0) )
for (i in 1:(NUM_STATES*(NUM_STATES-1))) q_obs[i] := q_obscomm
...
monitors.append( mnModel(filename="RES_HISSE_ST6NOULTRA_ALLEQ_1/model.log", printgen=1) )
...
monitors.append( mnStochasticCharacterMap(cdbdp=timetree,
					  printgen=1,
					  filename="RES_HISSE_ST6NOULTRA_ALLEQ_1/stoch_char_map.log",
					  include_simmap=true) )
monitors.append( mnScreen(printgen=1, lambda_obscomm, lambda_hid, q_obscomm, q_hid) )
